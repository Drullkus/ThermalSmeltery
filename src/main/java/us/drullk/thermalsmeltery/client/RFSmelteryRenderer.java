package us.drullk.thermalsmeltery.client;
...
public class RFSmelteryRenderer implements ISimpleBlockRenderingHandler
{
	public static int RFSmelteryModel = RenderingRegistry.getNextAvailableRenderId();
	...
	public boolean renderSmeltery(IBlockAccess world, int x, int y, int z, Block block, int modelID, RenderBlocks renderer)
	{
		...
		TileRFSmelteryLogic logic = (TileRFSmelteryLogic) world.getTileEntity(x, y, z);

		if(logic.validStructure)
		{
			CoordTuple from = logic.getMinCoordTuple();
			CoordTuple to = logic.getMaxCoordTuple();
			...
			if(logic.getSizeInventory() > 0)
			{
				...
				{
					renderLayer(logic, i * (logic.getDiameter() * logic.getDiameter()), from, to, from.y + i, renderer, world);
				}
			}
			...
			int yBase = 0;
			int liquidBase = 0;
			for(FluidStack liquid : logic.getFluidStorage())
			{
	...
	void renderLayer(TileRFSmelteryLogic logic, int start, CoordTuple from, CoordTuple to, int posY, RenderBlocks renderer, IBlockAccess world)
	{
